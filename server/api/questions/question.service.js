...
Question.retrieveAll = (err, result) => {
    ...
    query = `SELECT questions.id, question, question_description, questions.created_at  FROM (questions JOIN users ON questions.user_id = users.id ) ORDER BY questions.created_at DESC`
    pool.query(query,
        (err, results) => {
            if (err || results.length === 0) {
                console.log('error: ', err);
